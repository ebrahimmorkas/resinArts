...
const handleEditOrder = async (e) => {
  ...
  if (allZeroQuantity && products.length > 0) {
    ...
    try {
      const res = await axios.post(
        'https://api.simplyrks.cloud/api/order/reject-zero-quantity-order',
  ...
  try {
    ...
    const res = await axios.post(
      `https://api.simplyrks.cloud/api/order/edit-order/${order._id}`, 
...
const handleConfirmationConfirm = async () => {
  ...
  try {
    const res = await axios.post(
      `https://api.simplyrks.cloud/api/order/confirm-order-update`,
      {
        orderId: order._id,
        products: products,
...
const validateProductStock = async (product) => {
  try {
    const res = await axios.get(`https://api.simplyrks.cloud/api/product/${product.product_id}`, {
      withCredentials: true
    });
