...
const AbandonedCart = () => {
  ...
  if (loading) return <div className="flex items-center justify-center min-h-screen"><div className="text-lg text-gray-600">Loading...</div></div>;
          ...
          <div className="flex flex-col space-y-4">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
              <div className="flex items-center space-x-2 bg-gray-100 rounded-lg p-1">
                <button onClick={() => setActiveFilter('abandoned')} className={`px-4 py-2 rounded-md text-sm font-medium ${activeFilter === 'abandoned' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-600'}`}>
                ...
                <button onClick={() => setActiveFilter('all')} className={`px-4 py-2 rounded-md text-sm font-medium ${activeFilter === 'all' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-600'}`}>
          ...
          <div className="overflow-x-auto overflow-y-auto max-h-96">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50 sticky top-0 z-10">
                <tr>
                  <th className="px-6 py-3 text-center bg-gray-50 border-b" style={{minWidth:'60px'}}><input type="checkbox" checked={currentData.length > 0 && selectedRows.length === currentData.length} onChange={handleSelectAll} className="w-4 h-4 text-blue-600 rounded" /></th>
                  <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase bg-gray-50 border-b" style={{minWidth:'80px'}}>Sr No.</th>
                  <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase bg-gray-50 border-b" style={{minWidth:'150px'}}>Name</th>
                  <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase bg-gray-50 border-b" style={{minWidth:'200px'}}>Email</th>
                  <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase bg-gray-50 border-b" style={{minWidth:'140px'}}>WhatsApp</th>
                  <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase bg-gray-50 border-b" style={{minWidth:'140px'}}>Phone</th>
                  <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase bg-gray-50 border-b" style={{minWidth:'120px'}}>Items</th>
                  <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase bg-gray-50 border-b" style={{minWidth:'140px'}}>Time</th>
                  {selectedRows.length === 0 && <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase bg-gray-50 border-b" style={{minWidth:'180px'}}>Actions</th>}
                </tr>
              </thead>
            ...
            <div className="flex items-center space-x-2">
              ...
              <div className="flex space-x-1">
                {[...Array(Math.min(totalPages, 7))].map((_, i) => {
                  ...
                  return <button key={p} onClick={() => setCurrentPage(p)} className={`px-3 py-2 text-sm rounded-md ${currentPage === p ? 'bg-blue-600 text-white' : 'text-gray-500 hover:bg-gray-50'}`}>{p}</button>;
                })}
              </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] flex flex-col">
            ...
            <div className="px-6 py-4 overflow-y-auto flex-1">
              <div className="mb-6 p-4 bg-gray-50 rounded-lg"><h4 className="text-sm font-medium mb-3">Customer Info</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                  <div><span className="text-gray-600">Email:</span><span className="ml-2">{selectedCart.email}</span></div>
                  <div><span className="text-gray-600">Phone:</span><span className="ml-2">{selectedCart.phone_number || 'N/A'}</span></div>
                  <div><span className="text-gray-600">WhatsApp:</span><span className="ml-2">{selectedCart.whatsapp_number || 'N/A'}</span></div>
                  <div><span className="text-gray-600">Updated:</span><span className="ml-2">{new Date(selectedCart.last_updated).toLocaleString()}</span></div>
                </div>
              </div>
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
            ...
            <div className="px-6 py-4"><p className="text-sm text-gray-600">Delete this cart? This will also clear user's cart items. Cannot be undone.</p></div>
            <div className="px-6 py-4 border-t flex justify-end space-x-3">
              <button onClick={() => setShowDeleteModal(false)} className="px-4 py-2 border rounded-md hover:bg-gray-50">Cancel</button>
      ...
      {showDeleteMultipleModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
            ...
            <div className="px-6 py-4"><p className="text-sm text-gray-600">Delete {selectedRows.length} cart(s)? This will also clear users' cart items. Cannot be undone.</p></div>
            <div className="px-6 py-4 border-t flex justify-end space-x-3">
              <button onClick={() => setShowDeleteMultipleModal(false)} className="px-4 py-2 border rounded-md hover:bg-gray-50">Cancel</button>
