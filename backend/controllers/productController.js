...
const addProduct = async (req, res) => {
  try {
    ...
    const hasBasePrice = price && price !== '' && !isNaN(price) && price >= 0;
    const hasColorPrices = parsedColorVariants.some(v => v.price && v.price !== '' && !isNaN(v.price) && v.price >= 0);
    const hasSizePrices = parsedSizeVariants.some(v => v.price && v.price !== '' && !isNaN(v.price) && v.price >= 0);
    const hasPricingSections = parsedPricingSections.some(s =>
      s.priceRanges && s.priceRanges.some(r => r.retailPrice && r.retailPrice !== '' && !isNaN(r.retailPrice) && r.retailPrice >= 0)
    );

    if (!hasBasePrice && !hasColorPrices && !hasSizePrices && !hasPricingSections) {
      return res.status(400).json({ error: 'At least one price (base, color variant, size variant, or pricing combination) is required' });
    }
    ...
    const validatePriceRanges = (ranges, context) => {
      ...
      ranges.forEach((range, index) => {
        if (range.retailPrice || range.wholesalePrice || range.thresholdQuantity) {
          if (!range.retailPrice || !range.wholesalePrice || !range.thresholdQuantity) {
            throw new Error(`All fields (retailPrice, wholesalePrice, thresholdQuantity) are required for price range ${index + 1} in ${context}`);
          }
          if (isNaN(range.retailPrice) || range.retailPrice < 0) {
            throw new Error(`Invalid retail price in price range ${index + 1} in ${context}`);
          }
          if (isNaN(range.wholesalePrice) || range.wholesalePrice < 0) {
            throw new Error(`Invalid wholesale price in price range ${index + 1} in ${context}`);
          }
          if (isNaN(range.thresholdQuantity) || range.thresholdQuantity < 1) {
            throw new Error(`Invalid threshold quantity in price range ${index + 1} in ${context}`);
          }
          if (parseFloat(range.retailPrice) > parseFloat(range.wholesalePrice)) {
            throw new Error(`Retail price cannot be greater than wholesale price in price range ${index + 1} in ${context}`);
          }
        }
      });
    };
    ...
    if (hasBasePrice && parsedBasePriceRanges.length > 0) {
      if (parseFloat(parsedBasePriceRanges[0].retailPrice) !== parseFloat(price)) {
    ...
    parsedColorVariants.forEach((variant, index) => {
      if (variant.price && variant.priceRanges && variant.priceRanges.length > 0) {
        if (parseFloat(variant.priceRanges[0].retailPrice) !== parseFloat(variant.price)) {
    ...
    parsedSizeVariants.forEach((variant, index) => {
      ...
      if (variant.forAllColors === 'no' && (!variant.availableColors || variant.availableColors.length === 0)) {
        throw new Error(`At least one color must be selected for size variant ${index + 1} if not available for all colors`);
      }
    });
    ...
    parsedColorVariants.forEach((variant, index) => {
      ...
      if (variant.forAllSizes === 'no' && (!variant.availableSizes || variant.availableSizes.length === 0)) {
        throw new Error(`At least one size must be selected for color variant ${index + 1} if not available for all sizes`);
      }
    });
    ...
    const productData = {
      name,
      price: hasBasePrice ? parseFloat(price) : undefined,
